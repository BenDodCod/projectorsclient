...
2. [הגדרה ראשונית](#2-הגדרה-ראשונית)
   - [2.7 מצב פריסה מנוהל IT](#27-מצב-פריסה-מנוהל-it) *(חדש בגרסה 2.1.0)*
3. [הבנת הממשק](#3-הבנת-הממשק)
4. [שימוש יומיומי](#4-שימוש-יומיומי)
5. [תכונות מתקדמות](#5-תכונות-מתקדמות)
   - [עדכונים אוטומטיים](#עדכונים-אוטומטיים) *(חדש בגרסה 2.1.0)*
6. [הגדרות](#6-הגדרות)
...
> **פריסה מנוהלת IT:** אם מחלקת ה-IT שלך התקינה את היישום באמצעות מערכת פריסה שקטה, אשף ההגדרה **לא יופיע**. היישום ייפתח ישירות בחלון הראשי, עם הגדרות SQL Server של הארגון שלך וסיסמת מנהל. עבור ל[סעיף 3](#3-הבנת-הממשק) ללימוד הממשק. ראה [סעיף 2.7](#27-מצב-פריסה-מנוהל-it) לפרטים על משמעות הדבר עבורך.
...
### 2.7 מצב פריסה מנוהל IT

כאשר מחלקת ה-IT שלך פורסת את היישום באופן מרכזי, החוויה שונה מהתקנה עצמאית.

**מה משתנה:**

| תכונה | הגדרה עצמאית | מנוהל IT |
|--------|-------------|----------|
| אשף הגדרה ראשונית | ✅ מוצג | ⏭ מדולג |
| מסד נתונים SQL Server | אופציונלי | מוגדר מראש |
| סיסמת מנהל | אתה יוצר | IT מגדיר |
| הגדרות חיבור | ניתנות לעריכה | לקריאה בלבד |
| מצב מסד נתונים | ניתן לבחירה | נעול (SQL Server) |

**מה תראה כשתפתח את היישום לראשונה:**
- חלון ראשי נפתח ישירות (ללא אשף)
- מחוון מצב **"SQL Server"** גלוי (לא "Standalone")
- הגדרות חיבור ב-Settings ← Connection מאופרות (אפורות)
- המקרן/ים שלך עשויים להיות מוגדרים כבר

**מדוע הגדרות החיבור לקריאה בלבד:**

במצב מנוהל IT, צוות ה-IT של הארגון שלך מנהל את החיבור למסד נתונים SQL Server באופן מרכזי. זה מונע תצורה שגויה בטעות על פני מספר תחנות עבודה.

> **למשתמשי קצה:** אתה עדיין יכול לשלוט במקרנים, לצפות בהיסטוריה ולשנות את ההעדפות האישיות שלך (שפה, כפתורי ממשק, התנהגות אתחול). רק שדות חיבור מסד הנתונים נעולים.

> **צריך לשנות הגדרות SQL Server?** פנה למנהל ה-IT שלך. הם יכולים לפרוס מחדש עם תצורה מעודכנת.

**סיסמת המנהל שלך:**

IT הגדיר סיסמת מנהל בזמן הפריסה. אם לא קיבלת סיסמה זו, פנה למנהל ה-IT שלך. סיסמת המנהל נדרשת לגישה ללשונית האבטחה בהגדרות.
...
היישום בודק עדכונים באופן אוטומטי ומודיע לך כאשר גרסה חדשה זמינה.

**איך זה עובד:**
- בעת הפעלה, היישום בודק עדכונים ברקע בשקט (ללא עיכוב לתהליך העבודה שלך)
- אם גרסה חדשה זמינה, תופיע תיבת דו-שיח הודעה
- אתה בוחר אם להוריד ולהתקין — עדכונים אינם מאולצים לעולם

**אפשרויות עדכון שלך:**

| אפשרות | מה היא עושה |
|---------|------------|
| **עדכן עכשיו** | מוריד ומתקין ברקע; היישום מופעל מחדש כשמוכן |
| **הזכר לי מאוחר יותר** | סוגר את תיבת הדו-שיח; ישאל שוב בהפעלה הבאה |
| **דלג על גרסה זו** | מדכא גרסה ספציפית זו; לא ישאל עד לגרסה חדשה יותר |

**בדיקת עדכון ידנית:**

1. פתח **הגדרות** (Ctrl+,)
2. עבור ללשונית **כללי**
3. לחץ **בדוק עכשיו** בסעיף עדכונים

**ערוצי עדכון:**

| ערוץ | תיאור |
|------|-------|
| **יציב** (ברירת מחדל) | גרסאות מלאות, נבדקו ביסודיות |
| **בטא** | בדיקה לפני שחרור, עשויות להיות בעיות קלות |
| **אלפא** | גישה מוקדמת, השתמש רק אם IT ביקש |

> **פריסות מנוהלות IT:** מנהל ה-IT שלך שולט בהגדרות עדכון. אם בדיקת העדכון מושבתת, פנה ל-IT לעדכוני גרסה.

> **לפרטים נוספים:** ראה [README_UPDATES.md](../README_UPDATES.md)
...
> **מצב מנוהל IT:** אם היישום שלך נופרס על ידי IT, שדות השרת, הפורט, מסד הנתונים והאימות הם **לקריאה בלבד** (אפורים). זה צפוי — הארגון שלך מנהל הגדרות אלו באופן מרכזי. רק מנהלי IT יכולים לשנות את חיבור ה-SQL Server.
...
**גרסה 1.1** (נוכחית — 17 בפברואר 2026)
- מתאים לגרסת יישום 2.1.0
- נוסף: סעיף 2.7 — מצב פריסה מנוהל IT (פריסה שקטה, הגדרות לקריאה בלבד)
- נוסף: סעיף עדכונים אוטומטיים בתכונות מתקדמות
- עודכן: הגדרות חיבור — הערה על שדות לקריאה בלבד בפריסות מנוהלות IT

**גרסה 1.0** (12 בפברואר 2026)
...
*מדריך משתמש גרסה 1.1*
*עודכן לאחרונה: 17 בפברואר 2026*
*תואם ל-Enhanced Projector Control Application גרסה 2.1.0 ואילך*
